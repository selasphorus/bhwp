...
class PostTypeRegistrar
{
    ...
    public function bootstrap(): void
    {
		...
		add_filter('bhwp_active_post_types', function(array $cpts) use ($activePostTypes): array {
			return array_keys($activePostTypes);
		}, 10, 1);

		// Contribute CPT-specific taxonomy handlers (e.g., Habitat) to the unified registrar
		if (!empty($activePostTypes)) {
			add_filter('bhwp_register_taxonomy_handlers', function(array $taxArray) use ($activePostTypes): array {
