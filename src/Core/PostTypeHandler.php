...
abstract class PostTypeHandler extends BaseHandler
{
    ...
    public static function queryDefaults(): array
    {
        ...
        /** @var array $filtered */
        $filtered = apply_filters('bhwp_generic_query_defaults', $defaults, $spec);
        return $filtered;
    }
    ...
    public static function normalizeFilters(array $input): array
    {
        ...
        /** @var array $filtered */
        $filtered = apply_filters('bhwp_generic_normalize_filters', $normalized, $input, $spec);
        return $filtered;
    }
    ...
    public static function buildQueryParams(array $normalized): array
    {
        ...
        /** @var array $filtered */
        //$filtered = apply_filters('bhwp_generic_query_params', $params, $normalized, $spec);
        $filtered = $params; // tft
    ...
    public static function find(array $filters): array
    {
        ...
        /** @var array $filtered */
        //$filtered = apply_filters('bhwp_generic_result', $payload, $params, $normalized, static::getQuerySpec());
    ...
    public static function getHandlerClassForPostType(string $postType): ?string
    {
        ...
        $activePostTypeSlugs = (array) apply_filters('bhwp_active_post_types', []);
	...
	public static function getScopeFromRequest(array $atts = [], string $default = 'this_year')
	{
		...
		if (empty($qv)) { $qv = get_query_var('bhwp_scope'); }
		if (empty($qv) && isset($_GET['scope'])) { $qv = (string) $_GET['scope']; }
		if (empty($qv) && isset($_GET['bhwp_scope'])) { $qv = (string) $_GET['bhwp_scope']; }
